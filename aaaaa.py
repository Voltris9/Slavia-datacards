...
# =============================================================================
# BĚŽECKÉ METRIKY + ALIASY (KANONICKÁ JMÉNA)
# =============================================================================
RUN = [
...
# =============================================================================
# NORMALIZACE BĚŽECKÝCH DAT – auto fix (aliasy, dopočty, Position z herních)
# =============================================================================
import unicodedata, re

# Přímé mapování názvů z tvých exportů (Holandsko / další ligy se stejným layoutem)
CUSTOM_RUN_RENAME = {
    "Distance P90": "Total distance per 90",           # předpoklad: km/90
    "HSR Distance P90": "High-speed distance per 90",
    "HI Count P90": "High-intensity runs per 90",
    "Sprint Count P90": "Sprints per 90",

    # akcelerace/decelerace – ponecháme původní názvy, spočítáme níže:
    ...
    # Max speed – použijeme PSV-99 (peak speed) jako km/h
    "PSV-99": "Max speed (km/h)",

    # pokud by někdy přišla přímo průměrná rychlost:
    "Average speed (km/h)": "Average speed (km/h)",
}
...
def _postcompute_run_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Dopočty metrik ze standardních exportů (Holandsko & spol.)."""
    if df is None or df.empty: return df

    # Average speed (km/h) z M/min P90 → km/h = (m/min)*0.06
    if "Average speed (km/h)" not in df.columns:
        if "M/min P90" in df.columns:
            df["Average speed (km/h)"] = pd.to_numeric(df["M/min P90"], errors="coerce") * 0.06

    # Accelerations per 90 = Medium + High
    if "Accelerations per 90" not in df.columns:
        accel_cols = []
        for c in ["High Acceleration Count P90", "Medium Acceleration Count P90"]:
            if c in df.columns: accel_cols.append(pd.to_numeric(df[c], errors="coerce"))
        if accel_cols:
            s = accel_cols[0]
            for a in accel_cols[1:]: s = s.add(a, fill_value=0)
            df["Accelerations per 90"] = s

    # Decelerations per 90 = Medium + High
    if "Decelerations per 90" not in df.columns:
        decel_cols = []
        for c in ["High Deceleration Count P90", "Medium Deceleration Count P90"]:
            if c in df.columns: decel_cols.append(pd.to_numeric(df[c], errors="coerce"))
        if decel_cols:
            s = decel_cols[0]
            for a in decel_cols[1:]: s = s.add(a, fill_value=0)
...
def auto_fix_run_df(run_df: pd.DataFrame, game_df: pd.DataFrame) -> pd.DataFrame:
    """
    - přemapuje názvy podle CUSTOM_RUN_RENAME
    - LONG->WIDE přes ensure_run_wide (když přijde long formát)
    - dopočítá Average speed, Accels/Decels
    - doplní Position z herních dat, když chybí
    """
    if run_df is None or run_df.empty:
        return run_df

    # 1) přejmenování podle mappingu + aliasy ID sloupců
    run_df = _apply_run_renames(run_df.copy())

    # 2) LONG -> WIDE (pokud je long)
    run_df = ensure_run_wide(run_df)

    # 3) dopočty
    run_df = _postcompute_run_columns(run_df)

    # 4) doplnění Position z herních dat (pokud chybí)
    if "Position" not in run_df.columns and game_df is not None and not game_df.empty:
        ...
        if "Position" in g.columns and "Player" in g.columns:
            g_small = g[["Player","Position"]].dropna()
            g_small = g_small.groupby("Player", as_index=False).agg({"Position":"first"})
            run_df["_key"] = _std_player_series(run_df["Player"])
            g_small["_key"] = _std_player_series(g_small["Player"])
    ...
    for c in ["Player","Team","Position"]:
        if c in run_df.columns:
            run_df[c] = run_df[c].astype(str).str.strip()
...
def compute_run_scores(player_row: pd.Series, cz_run_agg: pd.DataFrame):
    ...
    run_scores, run_abs = {}, {}
    # speciální zacházení: pokud byl zdrojový sloupec "M/min P90", už jsme přepočítali na "Average speed (km/h)"
    for eng, label in RUN:
        val_abs = value_with_alias_run(player_row, eng)
        # fallback: pokud se hledá "Average speed (km/h)" a není, zkus M/min P90
        if pd.isna(val_abs) and eng == "Average speed (km/h)" and "M/min P90" in player_row.index:
            try:
                val_abs = float(pd.to_numeric(player_row["M/min P90"], errors="coerce")) * 0.06
            except Exception:
                val_abs = np.nan
...
with tab_card:
    ...
    # běžecké datasety – aliasy + long->wide + dopočty + doplnění Position z herních dat
    run_cz_df = pd.read_excel(run_cz_file) if run_cz_file else None
    run_players_df = pd.read_excel(run_players_file) if run_players_file else None

    run_cz_df = auto_fix_run_df(run_cz_df, league) if run_cz_df is not None else None
    run_players_df = auto_fix_run_df(run_players_df, players) if run_players_df is not None else None
