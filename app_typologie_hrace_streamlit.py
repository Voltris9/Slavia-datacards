...
if league_df is not None and player_df is not None and len(player_df) > 0:
    ...
    # -------------------- Vizualizace: RADAR (menší, přehledný) --------------------
    ...
    fig_radar, ax = plt.subplots(figsize=(5, 5), subplot_kw=dict(polar=True))  # menší radar
    ...
    ax.set_ylim(0,150)
    ax.set_title(f"{jmeno} – radar (% vs. liga)", fontsize=12)
    ...
    fig_hm, ax2 = plt.subplots(figsize=(4.2, 0.32*len(heat_labels)+0.8))  # kompaktnější
    im = ax2.imshow(hm_plot, cmap=cmap, vmin=0, vmax=150)
    ax2.set_yticks(range(len(heat_labels)))
    ax2.set_yticklabels(heat_labels, fontsize=9)
    ax2.set_xticks([])
    ...
    ax2.set_title(f"{jmeno} – komplexní činnosti (% vs. liga, škála 0–150)", fontsize=11)
    st.pyplot(fig_hm, use_container_width=True)

    # -------------------- Textový report (souvislý, přísný, delší) --------------------
    ...
    paragraphs = []
    paragraphs.append(
        f"{jmeno} ({int(vek) if not pd.isna(vek) else 'věk ?'}, {klub}) má v této sezoně odehráno "
        f"{int(minuty) if not pd.isna(minuty) else '?'} minut, vzorek je "
    ...
    if (not pd.isna(g90) and g90 < 70) and (not pd.isna(a90) and a90 < 70):
        ...
        prod_txt = "dobrá – gólová/finální produkce odpovídá či převyšuje ligový standard na pozici."
    else:
    ...
    if HAVE_DOCX:
        ...
        doc.add_picture(buf_radar, width=Inches(4.5))  # menší v DOCX

        doc.add_heading("Heatmapa (0–150 %)", level=1)
        doc.add_picture(buf_heat, width=Inches(3.8))  # menší v DOCX
