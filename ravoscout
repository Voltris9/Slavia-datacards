...
with tab_search:
    ...
    with c1:
        league_name = st.text_input("Název ligy (zobrazí se ve výstupu)", value="Cizí liga")
    with c2:
        min_minutes = st.number_input("Min. minut (pokud ve zdroji)", min_value=0, value=0, step=100)
    with c3:
        min_games = st.number_input("Min. zápasů (pokud ve zdroji)", min_value=0, value=0, step=1)

    run = st.button("Spustit vyhledávání")

    if run:
        if not cz_file or not fr_file:
            st.error("Nahraj oba soubory (CZ + cizí liga).")
            st.stop()
        try:
            cz_df = pd.read_excel(cz_file)
            fr_df = pd.read_excel(fr_file)
        except Exception as e:
            st.error(f"Chyba při načítání souborů: {e}")
            st.stop()
        ...
        if res_df.empty:
            st.warning("Nenašli jsme žádné ANO kandidáty pro zadané parametry.")
        else:
            ...
            st.download_button(
                ...
                file_name=f"kandidati_{league_name}.csv",
                mime="text/csv"
            )
            ...
            with zipfile.ZipFile(zbuf, "w", zipfile.ZIP_DEFLATED) as zf:
                for name, png_bytes in cards:
                    safe = str(name).replace("/","_").replace("\\","_")
                    zf.writestr(f"{safe}.png", png_bytes)
            st.download_button(
                "🗂️ Stáhnout všechny karty (ZIP)",
                data=zbuf.getvalue(),
                file_name=f"karty_{league_name}.zip",
                mime="application/zip"
            )

            # Náhled karty jednoho vybraného kandidáta
