...
def ensure_run_wide(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Metric" in df.columns and "Value" in df.columns:
        ...
        if pc and pc != "Player" and pc in wide.columns:
            wide = wide.rename(columns={pc: "Player"})
        sc = get_pos_col(df)
        if sc and sc != "Position" and sc in wide.columns:
            wide = wide.rename(columns={sc: "Position"})
        return wide
    pc = get_player_col(df)
    if pc and pc != "Player":
        df = df.rename(columns={pc: "Player"})
    sc = get_pos_col(df)
    if sc and sc != "Position":
        df = df.rename(columns={sc: "Position"})
    return df
...
def peer_avg(cz_agg, pg, weights, mweights):
    ...
    for nm in peers(pg):
        if nm in cz_agg.index:
            ...
            r["Player"]=nm; r["Position"]=pg
            _,s = section_scores(r,cz_agg,mweights)
            v = role_index(s,weights)
...
def _post_run(df):
    ...
    if "Accelerations per 90" not in df.columns:
        ...
        if acc_cols:
            ...
            for c in acc_cols[1:]:
                s = s.add(pd.to_numeric(df[c],errors="coerce"), fill_value=0)
...
def auto_fix_run_df(run_df:pd.DataFrame, game_df:pd.DataFrame)->pd.DataFrame:
    ...
    if "Position" not in run_df.columns and game_df is not None and not game_df.empty:
        ...
        if "Position" in g.columns and "Player" in g.columns:
            ...
            run_df["_k"]=run_df["Player"].map(_normtxt)
            g["_k"]=g["Player"].map(_normtxt)
...
def run_scores_for_row(row,pop_agg):
    if pop_agg is None or pop_agg.empty:
...
def render_card_visual(player,team,pos,age,scores,sec_index,overall_base,verdict,
    ...
    y0=0.88
    for title,lst,key in blocks:
        ax.text(0.02,y0,title,fontsize=15,fontweight="bold",va="top")
        y=y0-0.04; L,R=0.04,0.26
        for i,(_,lab) in enumerate(lst):
            val=scores[key].get(lab,np.nan)
            x=L if i%2==0 else R
    ...
    if run_scores is not None and RUN_KEY in run_scores and len(run_scores[RUN_KEY])>0:
        ...
        for i,(_,lab) in enumerate(RUN):
            v_pct=run_scores[RUN_KEY].get(lab,np.nan)
            v_abs=(run_abs or {}).get(lab,np.nan)
            x=L if i%2==0 else R
    ...
    for k in ["Defenziva","Ofenziva","Přihrávky","1v1"]:
        ...
        ax.text(0.56,y-0.005,f"{k}: {'n/a' if pd.isna(v) else str(int(round(v)))+'%'}",fontsize=13,va="center",ha="left")
        y-=0.075
...
def render_run_card(player,team,pos,age,run_scores,run_abs,run_index,verdict):
    ...
    for i,(_,lab) in enumerate(RUN):
        pct=run_scores[RUN_KEY].get(lab,np.nan)
        val=(run_abs or {}).get(lab,np.nan)
        x=L if i%2==0 else R
...
with st.sidebar.expander("Váhy metrik v sekcích (volitelné)",expanded=False):
    for title,lst,key in blocks:
        ...
        s=sum(tmp.values())
        metric_w[key]=None if s==0 else {lab:w/s for lab,w in tmp.items()}
...
with tab_card:
    ...
    run_scores,run_abs,run_index = None,None,np.nan
    if run_cz_df is not None and run_pl_df is not None:
        poscol_cz = get_pos_col(run_cz_df)
        plcol_pl  = get_player_col(run_pl_df)
        cz_run_pos = run_cz_df[poscol_cz].astype(str).str.contains(rgx,na=False,regex=True) if poscol_cz else pd.Series(False,index=run_cz_df.index)
        cand_mask  = run_pl_df[plcol_pl]==player if plcol_pl else pd.Series(False,index=run_pl_df.index)
        cz_pos_df = run_cz_df.loc[cz_run_pos] if poscol_cz else pd.DataFrame()
        cand_df   = run_pl_df.loc[cand_mask] if plcol_pl else pd.DataFrame()
        if not cz_pos_df.empty and not cand_df.empty:
            r_run=cand_df.iloc[0]
            plc = get_player_col(cz_pos_df) or "Player"
            cz_agg = (cz_pos_df.rename(columns={plc:"Player"}) if plc!="Player" and plc in cz_pos_df.columns else cz_pos_df).groupby("Player").mean(numeric_only=True)
            run_scores,run_abs,run_index = run_scores_for_row(r_run, cz_agg)

        with st.expander("Kontrola běžeckých dat",expanded=False):
            cz_agg_tmp=None
            if poscol_cz:
                cz_pos = run_cz_df[run_cz_df[poscol_cz].astype(str).str.contains(rgx,na=False,regex=True)]
                if not cz_pos.empty:
            ...
            miss_pl=[lab for eng,lab in RUN if pd.isna(run_val(cand_df.iloc[0] if not cand_df.empty else pd.Series(dtype=object),eng))]
...
with tab_search:
    ...
    if run:
        ...
        def search_candidates():
            ...
            for p in pos_sel:
                rgx=POS_REGEX[p]
                mask |= fr_df["Position"].astype(str).str.contains(rgx,na=False,regex=True)
            base=fr_df.loc[mask].copy()
            def pick(df,names): return next((n for n in names if n in df.columns),None)
            mc=pick(base,["Minutes","Minutes played","Min"])
            gc=pick(base,["Games","Matches"])
            if min_minutes and mc:
                base=base[pd.to_numeric(base[mc],errors="coerce").fillna(0)>=min_minutes]
            if min_games and gc:
                base=base[pd.to_numeric(base[gc],errors="coerce").fillna(0)>=min_games]

            rows=[]; cards=[]
            for _,r in base.iterrows():
                pg=pos_group(r.get("Position",""))
                rgx=POS_REGEX[pg]
                ...
                scores,sec_idx=section_scores(r,cz_agg,metric_w)
                overall=role_index(sec_idx,sec_w)

                run_idx=np.nan; r_scores=None; r_abs=None
                if cz_run_df is not None and fr_run_df is not None:
                    cz_run_pos=cz_run_df[get_pos_col(cz_run_df)].astype(str).str.contains(rgx,na=False,regex=True) if get_pos_col(cz_run_df) else pd.Series(False,index=cz_run_df.index if cz_run_df is not None else [])
                    cand=fr_run_df[get_player_col(fr_run_df)]==r.get("Player","") if get_player_col(fr_run_df) else pd.Series(False,index=fr_run_df.index if fr_run_df is not None else [])
                    cz_run_pos_df = cz_run_df.loc[cz_run_pos] if get_pos_col(cz_run_df) else pd.DataFrame()
                    cand_df = fr_run_df.loc[cand] if get_player_col(fr_run_df) else pd.DataFrame()
                    if not cz_run_pos_df.empty and not cand_df.empty:
                        r_run=cand_df.iloc[0]
                        plc=get_player_col(cz_run_pos_df) or "Player"
                        cz_run_agg=(cz_run_pos_df.rename(columns={plc:"Player"}) if plc!="Player" and plc in cz_run_pos_df.columns else cz_run_pos_df).groupby("Player").mean(numeric_only=True)
                ...
                baseVerd=final_idx if (not pd.isna(final_idx) and w_run>0) else overall
                ...
                if verdict.startswith("ANO"):
                    ...
                    rows.append({"Hráč":player,"Věk":age,"Klub":team,"Pozice":pos,"Liga":league_name,
                                 ...
                                 "Role-index (vážený)":overall,"Run index":run_idx,
                                 "Final index": final_idx if (not pd.isna(run_idx) and w_run>0) else overall,
                                 "Verdikt":verdict})
    ...
    if res_df is None or res_df.empty:
        ...
        if sel:
            ...
            r=fr_df.loc[fr_df["Player"]==sel].iloc[0]
            pg=pos_group(r.get("Position",""))
            rgx=POS_REGEX[pg]
            cz_pos=cz_df[cz_df["Position"].astype(str).str.contains(rgx,na=False,regex=True)]
            if not cz_pos.empty:
                ...
                if cz_run_df is not None and fr_run_df is not None:
                    poscol_cz=get_pos_col(cz_run_df)
                    if poscol_cz:
                        cz_run_pos=cz_run_df[poscol_cz].astype(str).str.contains(rgx,na=False,regex=True)
                        cz_run_pos_df=cz_run_df.loc[cz_run_pos]
                    else:
                        cz_run_pos_df=pd.DataFrame()
                    cand=fr_run_df[get_player_col(fr_run_df)]==sel if get_player_col(fr_run_df) else pd.Series(False,index=fr_run_df.index if fr_run_df is not None else [])
                    cand_df=fr_run_df.loc[cand] if get_player_col(fr_run_df) else pd.DataFrame()
                    if not cz_run_pos_df.empty and not cand_df.empty:
                        r_run=cand_df.iloc[0]
                        plc=get_player_col(cz_run_pos_df) or "Player"
                ...
                peer=peer_avg(cz_agg,pg,sec_w,metric_w)
...
with tab_run:
    ...
    c1,c2=st.columns(2)
    with c1:
        cz_run_file = st.file_uploader("CZ běžecká data – benchmark (xlsx)", type=["xlsx"], key="run_cz_only")
    with c2:
        any_run_file=st.file_uploader("Běžecká data – libovolná liga (xlsx)", type=["xlsx"], key="run_any_only")
    ...
    if cz_run_file and any_run_file:
        ...
        if get_pos_col(cz_run):
            rgx=POS_REGEX[pg]
            cz_pos=cz_run[cz_run[get_pos_col(cz_run)].astype(str).str.contains(rgx,na=False,regex=True)]
