...
def _postcompute_run_columns(df):
    if df is None or df.empty: return df
...
def ensure_run_wide(df):
    ...
    if ("Metric" in df.columns) and ("Value" in df.columns):
        ...
        return wide
    c = get_pos_col(df)
    if c and c!="Position": df = df.rename(columns={c:"Position"})
    c = get_player_col(df)
    if c and c!="Player": df = df.rename(columns={c:"Player"})
    return df
...
def auto_fix_run_df(run_df, game_df):
    ...
    if need_fill_position and game_df is not None and not game_df.empty:
        ...
        if ("Player" in g.columns) and ("Position" in g.columns):
            ...
            key_col = get_player_col(run_df) or "Player"
            run_df["_key"] = _std_player_series(run_df[key_col])
            g["_key"] = _std_player_series(g["Player"])
            run_df = run_df.merge(g[["_key","Position"]], on="_key", how="left")
            if "Position" not in run_df.columns and "Position_y" in run_df.columns:
                run_df = run_df.rename(columns={"Position_y":"Position"})
...
def find_run_row_by_player(run_df, player_name):
...
sec_weights = {}
for sec in default_weights:
    sec_weights[sec] = st.sidebar.slider(sec, 0, 100, default_weights[sec], 1)
...
with st.sidebar.expander("Váhy metrik v sekcích (volitelné)", expanded=False):
    ...
    for _, lst, key in blocks:
        tmp = {}
        for _, label in lst:
            tmp[label] = st.slider("– " + label, 0, 100, 10, 1, key=key + "_" + label)
        ssum = sum(tmp.values())
...
with tab_card:
    ...
    verdict = "ANO – potenciální posila do Slavie" if (not np.isnan(peer_avg) and not np.isnan(overall) and overall >= peer_avg) else "NE – nedosahuje úrovně slávistických konkurentů"
...
with tab_search:
    ...
    positions_selected = st.multiselect("Pozice", list(POS_REGEX.keys()), default=list(POS_REGEX.keys()))
    ...
    if run_btn:
        ...
        def search_candidates(cz_df, foreign_df, positions_selected, sec_weights, metric_weights,
                              ...
                              cz_run_df=None, fr_run_df=None, w_run=0.0):
            mask_pos = pd.Series([False]*len(foreign_df))
            for p in positions_selected:
                mask_pos = mask_pos | foreign_df["Position"].astype(str).str.contains(POS_REGEX[p], na=False, regex=True)
            ...
            for _,r in base.iterrows():
                ...
                final_index = overall if (pd.isna(run_idx) or w_run<=0) else (1.0 - w_run)*overall + w_run*run_idx
                peer_avg = avg_peer_index(cz_agg, pos_group, sec_weights, metric_weights)
                verdict = "ANO – potenciální posila do Slavie" if (not np.isnan(peer_avg) and not np.isnan(final_index) and final_index >= peer_avg) else "NE – nedosahuje úrovně slávistických konkurentů"
                ...
                if verdict.startswith("ANO"):
                    ...
                    rows.append({
                        ...
                        "Run index": run_idx,
                        "Final index": (final_index if (not pd.isna(run_idx) and w_run>0) else np.nan),
                        "Verdikt": verdict
                    })
                    fig = render_card_visual(player, team, pos, age, scores, sec_idx, overall, verdict,
                                             ...
                                             final_index=(final_index if (not pd.isna(run_idx) and w_run>0) else None), w_run=w_run)
        ...
        res_df, cards = search_candidates(
            cz_df, fr_df, positions_selected, sec_weights, metric_weights,
    ...
    if res_df is None or res_df.empty:
        ...
        st.success("Nalezeno kandidátů: " + str(len(res_df)))
        st.dataframe(res_df, use_container_width=True)

        st.download_button(
            "📥 Stáhnout CSV s kandidáty",
            data=res_df.to_csv(index=False).encode("utf-8-sig"),
            file_name="kandidati_" + str(st.session_state.get('search_league','liga')) + ".csv",
            mime="text/csv"
        )
        ...
        st.download_button("🗂️ Stáhnout všechny karty (ZIP)", data=zbuf.getvalue(),
                           file_name="karty_" + str(st.session_state.get('search_league','liga')) + ".zip",
        ...
        if sel and fr_df_cached is not None and cz_df_cached is not None:
            ...
            if not cz_pos.empty:
                ...
                final_index = None
                if (not pd.isna(run_idx)) and (w_run>0):
                    final_index = (1.0 - w_run)*overall + w_run*run_idx
                base_for_verdict = final_index if (final_index is not None) else overall
                peer_avg = avg_peer_index(cz_agg, pos_group, sec_weights, metric_weights)
                verdict = "ANO – potenciální posila do Slavie" if (not np.isnan(peer_avg) and not np.isnan(base_for_verdict) and base_for_verdict >= peer_avg) else "NE – nedosahuje úrovně slávistických konkurentů"
